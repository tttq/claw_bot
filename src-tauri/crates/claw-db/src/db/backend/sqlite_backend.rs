...
use crate::db::conn::{init_agent_tables, init_core_tables};
...
impl SqliteBackend {
    ...
    pub async fn initialize() -> Result<DatabaseInitResult, String> {
        ...
        if config.database.sqlite.enable_vec {
            vector_support = crate::vector_store::init_vector_extension(&conn)
                .await
                .unwrap_or(false);
        }
        ...
        let agent_conn = sea_orm::Database::connect(&agent_url)
            .await
        ...
        log::info!(
            "[SQLite] Initialization complete | created={} repaired={} vec={}",
            tables_created.len(),
            tables_repaired.len(),
            vector_support
        );
    ...
    pub async fn check_status() -> Result<DatabaseStatus, String> {
        ...
        for (name, status) in core_validation
            .into_iter()
            .chain(agent_validation.into_iter())
        {
