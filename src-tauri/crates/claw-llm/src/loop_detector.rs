...
impl LoopDetector {
    ...
    fn evaluate(&mut self) -> LoopStatus {
        if self.same_tool_count >= MAX_SAME_TOOL_CONSECUTIVE {
            log::warn!(
                "[LoopDetector] BROKEN: same tool '{}' repeated {} times consecutively",
                self.history
                    .last()
            ...
            return LoopStatus::Broken(format!(
                "same tool '{}' repeated {} times",
                self.history
                    .last()
                    .map(|t| t.tool_name.as_str())
                    .unwrap_or("?"),
        ...
        if self.is_ping_pong(2) || self.is_ping_pong(3) {
            log::warn!(
                "[LoopDetector] BLOCKED: ping-pong pattern detected (history len={})",
                self.history.len()
            );
        ...
        if self.no_progress_in_last_n(3) {
            ...
            if self.warning_count >= 2 {
                log::warn!(
                    "[LoopDetector] BLOCKED: no progress in last 3 calls, warnings={}",
                    self.warning_count
                );
    ...
    fn is_ping_pong(&self, period: usize) -> bool {
        ...
        for i in 0..period {
            if self.history[n - 1 - i].tool_name != self.history[n - 1 - period - i].tool_name {
                return false;
            }
            if self.history[n - 1 - i].args_hash != self.history[n - 1 - period - i].args_hash {
                return false;
            }
        }
        true
        ...
        for i in (0..=n).rev() {
            ...
            if preview_len > 10 {
                if let Some(bl) = base_len {
                    if (preview_len as isize - bl as isize).abs() > 200 {
                        return false;
                    }
                } else {
