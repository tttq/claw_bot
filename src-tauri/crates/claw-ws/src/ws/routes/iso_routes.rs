...
/// 列出所有隔离Agent
pub async fn iso_agent_list(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_agent_list_ws(&params).await {
...
pub async fn iso_agent_create(
    ...
    match ws_adapters::iso_agent_create_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 获取隔离Agent详情
pub async fn iso_agent_get(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_agent_get_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 重命名隔离Agent
pub async fn iso_agent_rename(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_agent_rename_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 删除隔离Agent
pub async fn iso_agent_delete(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_agent_delete_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 设置隔离Agent配置
pub async fn iso_set_config(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_set_config_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 获取隔离Agent配置
pub async fn iso_get_config(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_get_config_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
/// 初始化隔离Agent数据库
pub async fn iso_init_agent_db(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_init_agent_db_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
pub async fn iso_set_tools_config(
    ...
    match ws_adapters::iso_set_tools_config_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 设置隔离Agent技能启用
pub async fn iso_set_skills_enabled(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_set_skills_enabled_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
pub async fn iso_agent_update_config(
    ...
    match ws_adapters::iso_agent_update_config_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
pub async fn iso_create_session(
    ...
    match ws_adapters::iso_create_session_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 列出隔离Agent会话
pub async fn iso_list_sessions(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_list_sessions_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
pub async fn iso_index_workspace(
    ...
    match ws_adapters::iso_index_workspace_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 列出隔离Agent工作区
pub async fn iso_list_workspace(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_list_workspace_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 清理隔离Agent数据
pub async fn iso_cleanup(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_cleanup_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
/// 生成隔离Agent提示词
pub async fn iso_generate_prompt(
    Extension(_state): Extension<Arc<AppState>>,
    Json(params): Json<serde_json::Value>,
) -> Json<ApiResponse<serde_json::Value>> {
    match ws_adapters::iso_generate_prompt_ws(&params).await {
        Ok(d) => Json(ApiResponse::ok(d)),
        Err(e) => Json(ApiResponse::err(&e)),
    }
}
...
impl ClawRouter for IsoRoutes {
    fn router() -> Router {
        Router::new()
            ...
            .route(
                "/api/iso/agent-update-config",
                post(iso_agent_update_config),
            )
