...
pub trait TaskExecutor: Send + Sync {
    fn execute(
        ...
        task: &AgentTask,
        config: &AppConfig,
    ) -> impl std::future::Future<Output = AgentTaskResult> + Send;
}
...
impl TaskExecutor for LlmTaskExecutor {
    ...
    async fn execute(&self, task: &AgentTask, config: &AppConfig) -> AgentTaskResult {
        ...
        log::info!(
            "[AgentEngine] Executing task={} agent={}",
            task.task_id,
            task.agent_id
        );
...
impl LlmTaskExecutor {
    ...
    async fn run_llm_stream(
        ...
        let model = agent
            .model_override
            .as_deref()
            .unwrap_or(&config.model.default_model);
        let api_key = config
            .resolve_api_key()
            .map_err(|e| anyhow::anyhow!("API key error: {}", e))?;
...
pub async fn execute_agent_task(task: AgentTask) -> Result<serde_json::Value, String> {
    log::info!(
        "[AgentEngine] Validating agent exists for task={} agent={}",
        task.task_id,
        task.agent_id
    );
    ...
    let _agent = match agent_session::iso_agent_get(task.agent_id.clone()).await {
        ...
        Ok(None) => {
            return Err(format!(
                "Agent '{}' does not exist. Cannot execute task '{}'.",
                task.agent_id, task.task_id
            ));
        }
        Err(e) => {
            return Err(format!(
                "Failed to look up agent '{}': {}",
                task.agent_id, e
            ));
        }
    };
    ...
    tokio::spawn(async move {
        ...
        match &result.status {
            s if s == "completed" || s == "failed" => {
                log::info!(
                    "[AgentEngine] task={} completed with status={}",
                    task_clone.task_id,
                    s
                );
            }
