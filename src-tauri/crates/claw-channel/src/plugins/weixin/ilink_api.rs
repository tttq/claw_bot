...
impl ILinkClient {
    ...
    fn build_headers(&self) -> reqwest::header::HeaderMap {
        ...
        headers.insert(
            ...
            client_version.to_string().parse().unwrap(),
        );
        headers
    }
    ...
    pub async fn get_updates(
        ...
        sync_buf: &str,
        timeout_secs: u64,
    ) -> Result<(Vec<ILinkUpdate>, String), String> {
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| format!("get_updates parse failed: {}", e))?;

        let errcode = data.get("errcode").and_then(|v| v.as_i64()).unwrap_or(0);
        if errcode != 0 {
            return Err(format!(
                "iLink API error: errcode={}, msg={}",
                errcode,
                data.get("errmsg")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown")
        ...
        let updates: Vec<ILinkUpdate> = msgs
            .iter()
            .filter_map(|m| {
                Some(ILinkUpdate {
    ...
    pub async fn send_message(
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| format!("send_message parse failed: {}", e))?;
        let errcode = data.get("errcode").and_then(|v| v.as_i64()).unwrap_or(0);
        if errcode != 0 {
    ...
    pub async fn send_typing(&self, to_user_id: &str, typing_ticket: &str) -> Result<(), String> {
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| format!("send_typing parse failed: {}", e))?;
        let errcode = data.get("errcode").and_then(|v| v.as_i64()).unwrap_or(0);
        if errcode != 0 {
    ...
    pub async fn get_config(&self) -> Result<serde_json::Value, String> {
        ...
        resp.json()
            .await
            .map_err(|e| format!("get_config parse failed: {}", e))
    }
    ...
    pub async fn get_bot_qrcode(&self) -> Result<ILinkQRCode, String> {
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| format!("get_bot_qrcode parse failed: {}", e))?;
    ...
    pub async fn get_qrcode_status(&self, qrcode_id: &str) -> Result<ILinkQRStatus, String> {
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| format!("get_qrcode_status parse failed: {}", e))?;
    ...
    pub async fn get_upload_url(
        ...
        resp.json()
            .await
            .map_err(|e| format!("get_upload_url parse failed: {}", e))
    }
}
