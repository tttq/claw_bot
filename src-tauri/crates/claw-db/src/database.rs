...
use crate::db::{init_main_db, get_db, try_get_agent_db};
use sea_orm::prelude::*;
...
impl From<crate::db::entities::conversations::Model> for Conversation {
    fn from(m: crate::db::entities::conversations::Model) -> Self {
        Conversation {
            ...
            updated_at: m.updated_at, message_count: std::cmp::max(m.message_count, 0) as u64,
            agent_id: None,
        }
    }
}
...
impl From<crate::db::entities::messages::Model> for Message {
    fn from(m: crate::db::entities::messages::Model) -> Self {
        Message {
            ...
            content: m.content, timestamp: m.timestamp, token_count: m.token_count,
            embedding: m.embedding, is_error: m.is_error != 0, model: m.model,
            metadata: m.metadata,
        }
    }
}
...
impl Database {
    ...
    pub async fn init(db_path: &PathBuf) -> Result<(), anyhow::Error> {
        let db_path_str = db_path.to_str().ok_or_else(|| anyhow::anyhow!("db path is not valid UTF-8: {:?}", db_path))?;
        let db = init_main_db(db_path_str).await?;
        log::info!("[DB] 初始化主数据库表结构...");
        let backend = db.get_database_backend();
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS conversations (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS messages (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_messages_conv ON messages(conversation_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_messages_time ON messages(timestamp)")).await?;

        // ===== 增强记忆系统 v2 (Hindsight-inspired) =====
        log::info!("[DB] 初始化增强记忆系统表...");
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS memory_units (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS entities (
                id TEXT PRIMARY KEY,
                agent_id TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS unit_entities (
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS memory_links (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS entity_cooccurrences (
        ...
        db.execute(Statement::from_string(backend, "CREATE VIRTUAL TABLE IF NOT EXISTS memory_units_fts USING fts5(
                text,
        ...
        // 索引优化
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_agent ON memory_units(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_conv ON memory_units(conversation_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_fact_type ON memory_units(fact_type)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_occurred ON memory_units(occurred_at)")).await?;
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_layer ON memory_units(memory_layer)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_units_expires ON memory_units(expires_at)")).await?;
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_links_from ON memory_links(from_unit_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_links_to ON memory_links(to_unit_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_memory_links_type ON memory_links(link_type)")).await?;
        log::info!("[DB] 增强记忆系统表初始化完成");

        // ===== 定时任务表 =====
        log::info!("[DB] 初始化定时任务/钩子/凭证/规避规则表...");
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS cron_jobs (
                id TEXT PRIMARY KEY,
                agent_id TEXT,
                name TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_cron_jobs_agent ON cron_jobs(agent_id)")).await?;

        // ===== 事件钩子表 =====
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS hooks (
                id TEXT PRIMARY KEY,
                agent_id TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_hooks_agent ON hooks(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_hooks_event ON hooks(event_type)")).await?;

        // ===== 凭证池表 =====
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS credential_pool (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_credential_pool_provider ON credential_pool(provider)")).await?;

        // ===== 规避规则表 =====
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS avoidance_rules (
                id TEXT PRIMARY KEY,
                agent_id TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_avoidance_rules_agent ON avoidance_rules(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_avoidance_rules_category ON avoidance_rules(category)")).await?;
    ...
    pub async fn create_conversation(agent_id: Option<String>) -> Result<Conversation, anyhow::Error> {
        let db = get_db().await;
        let now = chrono::Utc::now().timestamp();
        let id = uuid::Uuid::new_v4().to_string();
        log::info!("[DB] INSERT conversation: id={}", claw_types::truncate_str_safe(&id, 16));
        ...
        if let Some(ref aid) = agent_id {
            match try_get_agent_db() {
                Some(agent_db) => {
                    ...
                    log::info!("[DB] INSERT agent_session for agent={} conv={}", claw_types::truncate_str_safe(&aid, 8), claw_types::truncate_str_safe(&id, 8));
                    let _ = crate::db::agent_entities::agent_sessions::ActiveModel {
                        ...
                        ..Default::default()
                    }.insert(agent_db).await;
                }
                None => {}
            }
        }

        Ok(Conversation { id, title: "New Conversation".into(), created_at: now, updated_at: now, message_count: 0, agent_id })
    }
    ...
    pub async fn list_conversations() -> Result<Vec<Conversation>, anyhow::Error> {
        ...
        let mut agent_map: std::collections::HashMap<String, String> = std::collections::HashMap::new();
        if let Some(agent_db) = try_get_agent_db() {
            if let Ok(sessions) = crate::db::agent_entities::agent_sessions::Entity::find()
                .filter(crate::db::agent_entities::agent_sessions::Column::ConversationId.is_not_null())
                .all(agent_db).await
            {
                for s in sessions {
        ...
        let result: Vec<Conversation> = models.into_iter().map(|m| {
            let aid = agent_map.get(&m.id).cloned();
            Conversation { id: m.id, title: m.title, created_at: m.created_at,
                updated_at: m.updated_at, message_count: std::cmp::max(m.message_count, 0) as u64,
                agent_id: aid }
        }).collect();
        Ok(result)
    ...
    pub async fn get_conversation(id: &str) -> Result<Option<Conversation>, anyhow::Error> {
        let db = get_db().await;
        log::info!("[DB] SELECT conversation WHERE id={}", claw_types::truncate_str_safe(id, 16));
        match Conversations::find_by_id(id.to_string()).one(db).await? {
            Some(m) => {
                ...
                Ok(Some(Conversation { id: m.id, title: m.title, created_at: m.created_at,
                    updated_at: m.updated_at, message_count: std::cmp::max(m.message_count, 0) as u64,
                    agent_id: aid }))
            }
            None => Ok(None),
    ...
    pub async fn get_conversation_agent_id(conversation_id: &str) -> Result<Option<String>, anyhow::Error> {
    ...
    pub async fn rename_conversation(id: &str, new_title: &str) -> Result<(), anyhow::Error> {
        let db = get_db().await;
        log::info!("[DB] UPDATE conversation SET title={} WHERE id={}", new_title, claw_types::truncate_str_safe(id, 16));
    ...
    pub async fn delete_conversation(id: &str) -> Result<(), anyhow::Error> {
        let db = get_db().await;
        log::info!("[DB] CASCADE DELETE conversation WHERE id={}", claw_types::truncate_str_safe(id, 16));
        ...
        if let Some(agent_db) = crate::db::try_get_agent_db() {
            if let Ok(_) = crate::db::agent_entities::agent_sessions::Entity::delete_many()
                ...
                .exec(agent_db).await
            {
                log::info!("[DB] Cleaned up agent_sessions for deleted conv {}", claw_types::truncate_str_safe(&id, 16));
            }
        }
    ...
    pub async fn add_message(conv_id: &str, role: &str, content: &str, model: Option<&str>, token_count: Option<i32>, metadata: Option<String>) -> Result<Message, anyhow::Error> {
        let db = get_db().await;
        let now = chrono::Utc::now().timestamp();
        let id = uuid::Uuid::new_v4().to_string();
        let is_error = role == "error";
        log::info!("[DB] INSERT message: conv={}, role={}, len={}, tokens={}", claw_types::truncate_str_safe(conv_id, 16), role, content.len(), token_count.unwrap_or(0));
        ...
        if let Some(conv) = Conversations::find_by_id(conv_id.to_string()).one(db).await? {
    ...
    pub async fn get_messages(conversation_id: &str) -> Result<Vec<Message>, anyhow::Error> {
        let db = get_db().await;
        log::info!("[DB] SELECT messages WHERE conv={}", claw_types::truncate_str_safe(conversation_id, 16));
    ...
    pub async fn export_all_data() -> Result<String, anyhow::Error> {
        ...
        let convs: Vec<Conversation> = Conversations::find()
            .order_by_desc(crate::db::entities::conversations::Column::UpdatedAt).all(db).await?
            .iter().map(|c| Conversation::from(c.clone())).collect();
        let mut all_msgs = Vec::new();
        for c in &convs { if let Ok(msgs) = Self::get_messages(&c.id).await { all_msgs.push((c.id.clone(), msgs)); } }
        ...
        let file_path = format!("claw_export_{}.json", chrono::Local::now().format("%Y%m%d_%H%M%S"));
    ...
    pub async fn import_data(path: &str) -> Result<(), anyhow::Error> {
        ...
        crate::db::entities::memory_units::Entity::delete_many().exec(db).await?;
        Conversations::delete_many().exec(db).await?;
        if let Some(convs) = data.get("conversations").and_then(|v| v.as_array()) {
            for c in convs {
                let id = c.get("id").and_then(|v| v.as_str()).unwrap_or("").to_string();
                let title = c.get("title").and_then(|v| v.as_str()).unwrap_or("Imported").to_string();
    ...
    pub async fn get_db_stats() -> Result<serde_json::Value, anyhow::Error> {
        ...
        let mem_count = crate::db::entities::memory_units::Entity::find().count(db).await? as i64;
        let db_path = std::path::PathBuf::from("claw.db");
        let file_size = std::fs::metadata(&db_path).ok().map(|m| m.len()).unwrap_or(0);
        Ok(serde_json::json!({ "conversations": conv_count, "messages": msg_count, "memory_units": mem_count, "db_size_bytes": file_size }))
    }
}
...
pub async fn clear_conversation_messages(conversation_id: String) -> Result<(), String> {
    let db = get_db().await;
    log::info!("[DB] 清空会话消息: conv={}", claw_types::truncate_str_safe(&conversation_id, 16));
    Messages::delete_many()
        .filter(crate::db::entities::messages::Column::ConversationId.eq(&conversation_id))
        .exec(db).await.map_err(|e| e.to_string())?;
    if let Some(conv) = Conversations::find_by_id(conversation_id.clone()).one(db).await.map_err(|e| e.to_string())? {
