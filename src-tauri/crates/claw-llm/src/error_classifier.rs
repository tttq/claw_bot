...
impl ErrorClassification {
    ...
    pub fn classify(status_code: u16, error_message: &str) -> Self {
        ...
        match status_code {
            ...
            400 if msg_lower.contains("context") || msg_lower.contains("token") || msg_lower.contains("too many") => Self {
            ...
            _ if msg_lower.contains("connection") || msg_lower.contains("network") || msg_lower.contains("dns") => Self {
            ...
            _ if msg_lower.contains("surrogate") || msg_lower.contains("encode") || msg_lower.contains("ascii") => Self {
    ...
    fn extract_retry_after(error_message: &str) -> Option<u64> {
        ...
        for keyword in &["retry-after:", "retry_after:", "retry after "] {
            if let Some(pos) = msg_lower.find(keyword) {
                ...
                let num_str: String = remaining.chars()
...
pub fn extract_reasoning(message: &Value) -> Option<String> {
    ...
    if let Some(details) = message.get("reasoning_details").and_then(|v| v.as_array()) {
        let parts: Vec<String> = details.iter()
            .filter_map(|d| d.get("text").and_then(|t| t.as_str()).map(|s| s.to_string()))
            .filter(|s| !s.is_empty())
            .collect();
        if !parts.is_empty() {
...
impl PromptCacheGuard {
    /// 创建新的提示缓存守卫实例
    pub fn new() -> Self {
        Self { locked: std::sync::Mutex::new(()) }
    }
