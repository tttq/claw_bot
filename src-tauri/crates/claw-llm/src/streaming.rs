...
static WS_EMIT_CALLBACK: OnceLock<Box<dyn Fn(&str, &str, serde_json::Value) + Send + Sync>> =
    OnceLock::new();

/// 设置WebSocket事件发射回调 — 用于同时通过WS推送流式事件
pub fn set_ws_emit_callback(
    callback: impl Fn(&str, &str, serde_json::Value) + Send + Sync + 'static,
) {
...
pub async fn call_openai_streaming(
                            ...
                            if let Some(tcs) = delta["tool_calls"].as_array() {
                                for tc in tcs {
                                    ...
                                    if let Some(name) = tc["function"]["name"].as_str() {
                                        entry.1 = restore_tool_name_from_api(name);
                                    }
    ...
    let reasoning_out = if reasoning_text.is_empty() {
        None
    } else {
        Some(reasoning_text)
    };
...
pub async fn call_anthropic_streaming(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        while let Some(line_end) = sse_buffer.find('\n') {
            ...
            if let Ok(evt) =
                serde_json::from_str::<serde_json::Value>(std::str::from_utf8(data).unwrap_or(""))
            {
                        ...
                        if let Some(cb) = evt.get("content_block") {
                            match cb["type"].as_str().unwrap_or("") {
                                ...
                                "tool_use" => {
                                    ...
                                    current_tool_name = restore_tool_name_from_api(
    ...
    let reasoning_out = if thinking_text.is_empty() {
        None
    } else {
        Some(thinking_text)
    };
