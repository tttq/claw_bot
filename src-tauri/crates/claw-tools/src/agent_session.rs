...
impl From<claw_db::db::agent_entities::agents::Model> for IsolatedAgent {
    fn from(m: claw_db::db::agent_entities::agents::Model) -> Self {
        IsolatedAgent {
            ...
            tools_config: m.tools_config.and_then(|s: String| serde_json::from_str(s.as_str()).ok()).unwrap_or(serde_json::json!({})),
            skills_enabled: m.skills_enabled.and_then(|s: String| serde_json::from_str(s.as_str()).ok()).unwrap_or(serde_json::json!([])),
...
/// 生成Agent唯一ID
fn generate_agent_id() -> String { Uuid::new_v4().to_string() }
...
impl AgentSessionManager {
    ...
    pub async fn init(agent_db_path: &std::path::Path) -> Result<(), anyhow::Error> {
        ...
        let db = claw_db::db::init_agent_db(agent_db_path.to_str().ok_or_else(|| anyhow::anyhow!("agent db path is not valid UTF-8: {:?}", agent_db_path))?).await?;
        log::info!("[AgentDB] 初始化表结构...");
        let backend = db.get_database_backend();
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agents (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE UNIQUE INDEX IF NOT EXISTS idx_agents_id ON agents(id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agent_sessions (
                id TEXT PRIMARY KEY,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_sessions_agent ON agent_sessions(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agent_configs (
        ...
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agent_workspace_files (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                agent_id TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_ws_agent ON agent_workspace_files(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agent_vectors (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                agent_id TEXT NOT NULL,
        ...
        db.execute(Statement::from_string(backend, "CREATE INDEX IF NOT EXISTS idx_vectors_agent ON agent_vectors(agent_id)")).await?;
        db.execute(Statement::from_string(backend, "CREATE TABLE IF NOT EXISTS agent_profiles (
        ...
        db.execute(Statement::from_string(backend, "PRAGMA journal_mode=WAL")).await?;
        db.execute(Statement::from_string(backend, "PRAGMA foreign_keys=ON")).await?;
    ...
    pub async fn create_isolated_agent_ext(
        ...
        purpose: Option<&str>, scope: Option<&str>, category: Option<&str>, model_override: Option<&str>,
    ) -> Result<IsolatedAgent, anyhow::Error> {
        let db = get_agent_db().await;
        let now = chrono::Utc::now().timestamp();
        let id = generate_agent_id();
        log::info!("[AgentDB] CREATE agent: name={}, id={}, category={:?}", name, claw_types::truncate_str_safe(&id, 16), category);

        let effective_prompt = if system_prompt.is_empty() {
            let cat = category.unwrap_or("general");
            let role_hint = match cat {
                "code" => format!("You are {}, an expert coding assistant. You write clean, efficient, and well-tested code. You follow best practices and explain your reasoning.", name),
                "search" => format!("You are {}, a research and information retrieval specialist. You find, analyze, and synthesize information from multiple sources.", name),
                "analysis" => format!("You are {}, an analytical thinker who breaks down complex problems into structured components. You provide data-driven insights.", name),
                "creative" => format!("You are {}, a creative assistant skilled in writing, brainstorming, and generating innovative ideas.", name),
        ...
        let new_agent = claw_db::db::agent_entities::agents::ActiveModel {
            ...
            description: Set(if description.is_empty() { None } else { Some(description.to_string()) }),
        ...
        Ok(IsolatedAgent {
            id: id.clone(), display_name: name.to_string(),
            ...
            tools_config: serde_json::json!({"enabled": []}), skills_enabled: serde_json::json!([]),
            max_turns: 20, temperature: Some(0.7), is_active: true,
            created_at: now, updated_at: now, conversation_count: 0, total_messages: 0,
            workspace_path: None,
        })
    }
    ...
    pub async fn get_agent(agent_id: &str) -> Result<Option<IsolatedAgent>, anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] SELECT agent WHERE id={}", claw_types::truncate_str_safe(agent_id, 16));
    ...
    /// 重命名Agent — 更新显示名称
    pub async fn rename_agent(agent_id: &str, new_name: &str) -> Result<IsolatedAgent, anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] UPDATE agent SET display_name={} WHERE id={}", new_name, claw_types::truncate_str_safe(agent_id, 16));
        let agent = Agents::find_by_id(agent_id.to_string()).one(db).await?
    ...
    pub async fn delete_agent(agent_id: &str) -> Result<(), anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] CASCADE DELETE agent WHERE id={}", claw_types::truncate_str_safe(agent_id, 16));
        ...
        AgentWorkspaceFiles::delete_many()
            .filter(claw_db::db::agent_entities::agent_workspace_files::Column::AgentId.eq(agent_id))
            .exec(db).await?;
    ...
    pub async fn set_tools_config(agent_id: &str, config: &serde_json::Value) -> Result<(), anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] UPDATE agent SET tools_config={} WHERE id={}", config, claw_types::truncate_str_safe(agent_id, 16));
        let agent = Agents::find_by_id(agent_id.to_string()).one(db).await?
            .ok_or_else(|| anyhow::anyhow!("Agent not found"))?;
        let mut active: claw_db::db::agent_entities::agents::ActiveModel = agent.into();
    ...
    /// 设置Agent启用的技能列表
    pub async fn set_skills_enabled(agent_id: &str, enabled: &[String]) -> Result<(), anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] UPDATE agent SET skills_enabled={:?} WHERE id={}", enabled, claw_types::truncate_str_safe(agent_id, 16));
        let agent = Agents::find_by_id(agent_id.to_string()).one(db).await?
            .ok_or_else(|| anyhow::anyhow!("Agent not found"))?;
        let mut active: claw_db::db::agent_entities::agents::ActiveModel = agent.into();
    ...
    pub async fn set_config(agent_id: &str, key: &str, value: &str) -> Result<(), anyhow::Error> {
        let db = get_agent_db().await;
        let now = chrono::Utc::now().timestamp();
        log::info!("[AgentDB] UPSERT config: agent={}, key={}", claw_types::truncate_str_safe(agent_id, 16), key);
        let existing: Option<claw_db::db::agent_entities::agent_configs::Model> = AgentConfigs::find()
            .filter(claw_db::db::agent_entities::agent_configs::Column::AgentId.eq(agent_id))
    ...
    pub async fn get_config(agent_id: &str, key: &str) -> Result<Option<String>, anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] SELECT config: agent={}, key={}", claw_types::truncate_str_safe(agent_id, 16), key);
        let result: Option<claw_db::db::agent_entities::agent_configs::Model> = AgentConfigs::find()
            .filter(claw_db::db::agent_entities::agent_configs::Column::AgentId.eq(agent_id))
            .filter(claw_db::db::agent_entities::agent_configs::Column::ConfigKey.eq(key))
            .one(db).await?;
    ...
    async fn get_global_default_for_key(key: &str) -> Option<String> {
        let config = claw_config::config::AppConfig::load_or_create(&claw_config::path_resolver::config_path()).ok()?;
        ...
        match key {
            ...
            "agent_stream_mode" => Some(if model.stream_mode { "true".to_string() } else { "false".to_string() }),
            _ => None,
        }
    }
    ...
    pub async fn create_session(agent_id: &str, conversation_id: Option<&str>) -> Result<AgentSession, anyhow::Error> {
        let db = get_agent_db().await;
        let now = chrono::Utc::now().timestamp();
        let sid = generate_agent_id();
        log::info!("[AgentDB] INSERT session: agent={}, conv={}", claw_types::truncate_str_safe(agent_id, 16), conversation_id.unwrap_or("none"));
    ...
    pub async fn list_sessions(agent_id: &str) -> Result<Vec<AgentSession>, anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] SELECT sessions WHERE agent_id={}", claw_types::truncate_str_safe(agent_id, 16));
    ...
    pub async fn index_workspace_file(agent_id: &str, relative_path: &str, full_path: &str, file_size: i64, content_type: Option<&str>) -> Result<(), anyhow::Error> {
        let db = get_agent_db().await;
        let now = chrono::Utc::now().timestamp();
        log::info!("[AgentDB] UPSERT workspace_file: agent={}, path={}", claw_types::truncate_str_safe(agent_id, 16), relative_path);
        let existing: Option<claw_db::db::agent_entities::agent_workspace_files::Model> = AgentWorkspaceFiles::find()
            .filter(claw_db::db::agent_entities::agent_workspace_files::Column::AgentId.eq(agent_id))
            .filter(claw_db::db::agent_entities::agent_workspace_files::Column::RelativePath.eq(relative_path))
            .one(db).await?;
        match existing {
            Some(m) => {
                let mut am: claw_db::db::agent_entities::agent_workspace_files::ActiveModel = m.into();
    ...
    pub async fn list_workspace_index(agent_id: &str) -> Result<Vec<(String, String, i64)>, anyhow::Error> {
        let db = get_agent_db().await;
        log::info!("[AgentDB] SELECT workspace_files WHERE agent_id={}", claw_types::truncate_str_safe(agent_id, 16));
        let files: Vec<claw_db::db::agent_entities::agent_workspace_files::Model> = AgentWorkspaceFiles::find()
            .filter(claw_db::db::agent_entities::agent_workspace_files::Column::AgentId.eq(agent_id))
            .order_by_asc(claw_db::db::agent_entities::agent_workspace_files::Column::RelativePath)
            .all(db).await?;
        Ok(files.into_iter().map(|f| (f.relative_path, f.full_path, f.file_size)).collect())
    }
...
pub async fn iso_init_agent_db(_app_handle: tauri::AppHandle) -> Result<(), String> {
    AgentSessionManager::init(claw_config::path_resolver::agent_db_path().as_path()).await.map_err(|e| e.to_string())
}
...
pub async fn iso_agent_create(
    ...
    AgentSessionManager::create_isolated_agent_ext(&name, &description, &systemPrompt, purpose.as_deref(), scope.as_deref(), category.as_deref(), modelOverride.as_deref()).await.map_err(|e| e.to_string())
}

/// Tauri命令：列出所有Agent
#[tauri::command]
pub async fn iso_agent_list() -> Result<Vec<IsolatedAgent>, String> {
    AgentSessionManager::list_all_agents().await.map_err(|e| e.to_string())
}

/// Tauri命令：获取指定Agent
#[tauri::command]
pub async fn iso_agent_get(id: String) -> Result<Option<IsolatedAgent>, String> {
    AgentSessionManager::get_agent(&id).await.map_err(|e| e.to_string())
}
...
pub async fn iso_agent_rename(id: String, newName: String) -> Result<IsolatedAgent, String> {
    AgentSessionManager::rename_agent(&id, &newName).await.map_err(|e| e.to_string())
}

/// Tauri命令：删除Agent
#[tauri::command]
pub async fn iso_agent_delete(id: String) -> Result<(), String> {
    AgentSessionManager::delete_agent(&id).await.map_err(|e| e.to_string())
}

/// Tauri命令：设置Agent工具配置
#[tauri::command]
#[allow(non_snake_case)]
pub async fn iso_set_tools_config(agentId: String, config: serde_json::Value) -> Result<(), String> {
    AgentSessionManager::set_tools_config(&agentId, &config).await.map_err(|e| e.to_string())
}
...
pub async fn iso_set_skills_enabled(agentId: String, enabled: Vec<String>) -> Result<(), String> {
    AgentSessionManager::set_skills_enabled(&agentId, &enabled).await.map_err(|e| e.to_string())
}
...
pub async fn iso_agent_update_config(
    ...
    let existing = claw_db::db::agent_entities::agents::Entity::find_by_id(agentId.clone())
        .one(db).await.map_err(|e: sea_orm::DbErr| e.to_string())?
    ...
    if let Some(sp) = systemPrompt { am.system_prompt = Set(Some(sp)); }
    else { am.system_prompt = Set(existing.system_prompt); }

    if let Some(p) = purpose { am.purpose = Set(Some(p)); }
    else { am.purpose = Set(existing.purpose); }

    if let Some(s) = scope { am.scope = Set(Some(s)); }
    else { am.scope = Set(existing.scope); }

    if let Some(m) = modelOverride { am.model_override = Set(Some(m)); }
    else { am.model_override = Set(existing.model_override); }

    if let Some(t) = maxTurns { am.max_turns = Set(t as i32); }
    else { am.max_turns = Set(existing.max_turns); }

    if temperature.is_some() { am.temperature = Set(temperature); }
    else { am.temperature = Set(existing.temperature); }
    ...
    am.update(db).await.map_err(|e: sea_orm::DbErr| e.to_string())?;
    Ok(())
}
...
pub async fn iso_set_config(agentId: String, key: String, value: String) -> Result<(), String> {
    AgentSessionManager::set_config(&agentId, &key, &value).await.map_err(|e| e.to_string())
}
...
pub async fn iso_get_config(agentId: String, key: String) -> Result<Option<String>, String> {
    AgentSessionManager::get_config(&agentId, &key).await.map_err(|e| e.to_string())
}

/// Tauri命令：创建Agent会话
#[tauri::command]
#[allow(non_snake_case)]
pub async fn iso_create_session(agentId: String, conversationId: Option<String>) -> Result<AgentSession, String> {
    AgentSessionManager::create_session(&agentId, conversationId.as_deref()).await.map_err(|e| e.to_string())
}
...
pub async fn iso_list_sessions(agentId: String) -> Result<Vec<AgentSession>, String> {
    AgentSessionManager::list_sessions(&agentId).await.map_err(|e| e.to_string())
}

/// Tauri命令：索引Agent工作区文件
#[tauri::command]
#[allow(non_snake_case)]
pub async fn iso_index_workspace(agentId: String, relativePath: String, fullPath: String, fileSize: i64, contentType: Option<String>) -> Result<(), String> {
    AgentSessionManager::index_workspace_file(&agentId, &relativePath, &fullPath, fileSize, contentType.as_deref()).await.map_err(|e| e.to_string())
}
...
pub async fn iso_list_workspace(agentId: String) -> Result<Vec<(String, String, i64)>, String> {
    AgentSessionManager::list_workspace_index(&agentId).await.map_err(|e| e.to_string())
}
...
pub async fn iso_cleanup(daysThreshold: i64) -> Result<u64, String> {
    AgentSessionManager::cleanup_stale_data(daysThreshold).await.map_err(|e| e.to_string())
}
