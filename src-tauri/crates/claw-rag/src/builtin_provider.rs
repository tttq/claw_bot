...
use crate::memory_pipeline::{MemoryPipeline, IngestionRequest};
use crate::rag::{hybrid_retrieve, build_rag_context};
...
impl MemoryProvider for BuiltinMemoryProvider {
    ...
    async fn retrieve(&self, agent_id: &str, query: &str, conversation_id: Option<&str>, limit: usize) -> Result<Vec<MemoryUnit>, String> {
        ...
        Ok(enhanced.into_iter().map(|e| MemoryUnit {
            id: e.id, text: e.text, fact_type: e.fact_type,
            context: e.context, occurred_at: e.occurred_at,
            source_type: e.source_type, tags: e.tags,
            importance_score: e.importance_score, semantic_score: e.semantic_score,
            bm25_score: e.bm25_score, temporal_score: e.temporal_score,
            final_score: e.final_score,
            memory_layer: e.metadata.as_deref()
    ...
    async fn store_interaction(&self, agent_id: &str, conversation_id: Option<&str>, user_msg: &str, assistant_msg: &str) -> Result<(), String> {
        let results = MemoryPipeline::ingest_interaction(agent_id, conversation_id, user_msg, assistant_msg).await;
    ...
    async fn retrieve_by_layers(&self, request: &LayerAwareRetrieveRequest) -> Result<Vec<MemoryUnit>, String> {
        ...
        Ok(layered.into_iter().map(|u| MemoryUnit {
            id: u.id,
            text: u.text,
    ...
    async fn cleanup_expired(&self, agent_id: &str) -> Result<u64, String> {
        ...
        if count > 0 {
            log::info!("[BuiltinProvider:cleanup_expired] Cleaned {} expired memories for agent={}", count, &agent_id[..8.min(agent_id.len())]);
        }
        Ok(count)
    }
    ...
    async fn get_layer_stats(&self, agent_id: &str) -> Result<Vec<LayerStatEntry>, String> {
        ...
        let mut layer_data: std::collections::HashMap<MemoryLayer, (usize, f64)> = std::collections::HashMap::new();
        for unit in &all_units {
            let layer = unit.memory_layer.as_deref()
                .map(|s| MemoryLayer::from_str(s))
                .unwrap_or_else(|| classify_to_layer(
                    &unit.fact_type, &unit.source_type, unit.tags.as_deref(), unit.importance_score
                ));
