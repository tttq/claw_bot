...
macro_rules! lock_store {
    ($store:expr) => {
        match $store.lock() {
            ...
            Err(e) => {
                log::error!(
                    "[Agent:lock_store] Mutex poisoned for {}: {}, recovering...",
                    stringify!($store),
                    e
                );
                e.into_inner()
...
static TASK_STORE: LazyLock<Mutex<HashMap<String, Value>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
/// 全局定时任务存储: name → {schedule, task, enabled, ...}
static CRON_STORE: LazyLock<Mutex<HashMap<String, Value>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
/// 全局 Todo 列表
...
pub async fn tool_agent(
    ...
    let config = claw_config::config::get_config()
        .await
    ...
    match m.as_str() {
        "background" | "async" => {
            ...
            lock_store!(TASK_STORE).insert(
                id.clone(),
                json!({"prompt": pr, "status": "running", "mode": m, "agent_id": agent_id}),
            );
            ...
            let out = format!(
                "后台代理已启动 [ID:{}]\n{}",
                id,
                claw_types::truncate_str_safe(&prompt, 200)
            );
            Ok(json!({"tool":"Agent","success":true,"output":out}))
        }
        _ => {
            let is_openai = config.is_openai_compatible();
            match call_llm_via_trait(
                &api_key,
                &base_url,
                &model,
                &system_prompt,
...
async fn call_llm_via_trait(
    ...
    if let Some(caller) = claw_traits::get_llm_caller() {
        caller
            .call_once(api_key, base_url, model, system, user_msg, is_openai)
            .await
    } else {
...
async fn call_llm_once_fallback(
    api_key: &str,
    base_url: &str,
    model: &str,
    system: &str,
...
pub fn tool_todo_write(todos: Value) -> Result<serde_json::Value, String> {
    ...
    *TODO_STORE
        .lock()
        .map_err(|e| format!("Lock poisoned: {}", e))? = list.clone();
    ...
    let done = list
        .iter()
        .filter(|t| t.get("status") == Some(&json!("completed")))
        .count();
    let pct = if list.is_empty() {
        0.0
    } else {
        (done as f64 / list.len() as f64) * 100.0
    };
    let out = format!(
        "{}\n\nProgress: {}/{} ({:.0}%)",
        lines.join("\n"),
...
pub fn tool_todo_get() -> Result<serde_json::Value, String> {
    ...
    if store.is_empty() {
        return Ok(json!({"tool":"TodoGet","success":true,"output":"No todos"}));
    }
...
pub fn tool_task_create(prompt: String) -> Result<serde_json::Value, String> {
    ...
    lock_store!(TASK_STORE).insert(
        id.clone(),
        json!({"id":id,"prompt":prompt,"status":"running"}),
    );
    let out = format!(
        "Task created [ID:{}]\n{}",
        id,
...
pub fn tool_task_get(task_id: String) -> Result<serde_json::Value, String> {
    ...
    match store.get(&task_id) {
        Some(t) => Ok(
            json!({"tool":"TaskGet","success":true,"output":serde_json::to_string(t).unwrap_or_default()}),
        ),
        None => {
            let out = format!("Task '{}' not found", task_id);
            Ok(json!({"tool":"TaskGet","success":false,"output":out}))
        }
    }
}
...
pub fn tool_task_list(status_filter: Option<String>) -> Result<serde_json::Value, String> {
    ...
    let tasks: Vec<&Value> = match &status_filter {
        Some(f) => s
            .values()
            .filter(|t| t.get("status") == Some(&json!(f)))
            .collect(),
        None => s.values().collect(),
    };
    if tasks.is_empty() {
        return Ok(json!({"tool":"TaskList","success":true,"output":"No background tasks"}));
    }
    let items: Vec<String> = tasks
        ...
        .map(|t| {
            ...
            format!(
                ...
                st,
                tid,
                claw_types::truncate_str_safe(pr, 80)
            )
        })
        .collect();
...
/// 工作流编排工具 — 多步骤顺序执行
#[tauri::command]
pub fn tool_workflow(
    name: String,
    steps: Option<Value>,
    inputs: Option<Value>,
) -> Result<serde_json::Value, String> {
    let steps_list: Vec<Value> = steps
        .and_then(|s| s.as_array().cloned())
        .unwrap_or_default();
    let results: Vec<String> = steps_list
        .iter()
        .enumerate()
        .map(|(i, st)| {
            format!(
                "Step {}: action={}",
                i + 1,
                st.get("action").and_then(|v| v.as_str()).unwrap_or("")
            )
        })
        .collect();
...
pub fn tool_enter_plan_mode() -> Result<serde_json::Value, String> {
    *lock_store!(PLAN_MODE) = true;
    Ok(
        json!({"tool":"EnterPlanMode","success":true,"output":"Plan mode activated! AI will plan before changes."}),
    )
}
...
pub fn tool_exit_plan_mode() -> Result<serde_json::Value, String> {
    *PLAN_MODE
        .lock()
        .map_err(|e| format!("Lock poisoned: {}", e))? = false;
    Ok(
        json!({"tool":"ExitPlanMode","success":true,"output":"Exited plan mode. Real operations enabled."}),
    )
}
...
pub fn tool_brief(
    ...
    let att = attachments
        .map(|a| format!("Attachments: {}", a.join(", ")))
        .unwrap_or_default();
    let out = format!("{}\n{}", message, att);
...
pub fn tool_config(
    ...
    match action.as_str() {
        ...
        "set" => {
            ...
            let out = format!(
                "Config change request: {} = {}\nSave via Settings panel",
                k, v
            );
            Ok(json!({"tool":"Config","success":true,"output":out}))
        }
        _ => {
            let out = format!("Unknown action: {}", action);
            Ok(json!({"tool":"Config","success":false,"output":out}))
        }
    }
}
...
#[tauri::command]
pub fn tool_notebook_edit(
    file_path: String,
    cell_index: u64,
    source: Option<Value>,
) -> Result<serde_json::Value, String> {
    let src_lines = source
        .as_ref()
    ...
    let out = format!(
        "Notebook Edit:\n  File: {}\n  Cell #{}\n  Source: {} lines",
        file_path, cell_index, src_lines
    );
...
pub fn tool_schedule_cron(
    ...
    let out = format!(
        "Cron registered: {} | Schedule: {} | Task: {} | Enabled: {}",
        name, schedule, task, en
    );
...
pub fn tool_schedule_list() -> Result<serde_json::Value, String> {
    ...
    if s.is_empty() {
        return Ok(json!({"tool":"ScheduleList","success":true,"output":"No scheduled tasks"}));
    }
    let items: Vec<String> = s
        .values()
        .map(|v| {
            let n = v.get("name").and_then(|x| x.as_str()).unwrap_or("?");
            let en = if v.get("enabled").and_then(|x| x.as_bool()).unwrap_or(false) {
                "ON"
            } else {
                "OFF"
            };
...
pub fn tool_ask_user_question(questions: Value) -> Result<serde_json::Value, String> {
    let qlist = questions
        .as_array()
        .ok_or("questions must be array".to_string())?;
    let mut ans = Vec::new();
    ...
    let out = format!(
        "Questions pending user response:\n{}\n\nUser will answer in the frontend UI.",
        ans.join("\n")
    );
...
pub async fn tool_create_tool(params: Value) -> Result<serde_json::Value, String> {
    ...
    if let Some(ref script) = handler_script {
        ...
        std::fs::write(&script_path, script)
            .map_err(|e| format!("Failed to write handler script: {}", e))?;
    ...
    crate::tool_registry::register_tool(
        def,
        crate::tool_registry::ToolSource::Extension,
        handler_script.clone(),
    )
    .await;

    log::info!(
        "[CreateTool] Extension tool '{}' created at {}",
        name,
        ext_dir.display()
    );
    ...
    if let Err(e) = claw_rag::rag::add_tool_memory(&name, &description, "extension").await {
        log::warn!(
            "[CreateTool] Failed to add tool memory for '{}': {}",
            name,
            e
        );
    }
...
"#,
    ...
    let skill_def = crate::skill_loader::load_skill_from_file(
        &skill_path,
        crate::skill_loader::SkillSource::User,
    );
    if let Some(loaded) = skill_def {
        crate::skill_loader::register_skills_as_tools(&[loaded]).await;
        log::info!(
            "[CreateSkill] Skill '{}' created and registered at {}",
            name,
            skill_dir.display()
        );
    } else {
        log::warn!(
            "[CreateSkill] Skill '{}' created but failed to register: {:?}",
            name,
            skill_def
        );
    }
    ...
    if let Err(e) =
        claw_rag::rag::add_skill_memory(&name, &description, when_to_use, &allowed_tools).await
    {
        log::warn!(
            "[CreateSkill] Failed to add skill memory for '{}': {}",
            name,
            e
        );
    }
