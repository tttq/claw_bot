...
pub use super::streaming::{emit_chat_stream, set_ws_emit_callback};

static HTTP_CLIENT: std::sync::OnceLock<reqwest::Client> = std::sync::OnceLock::new();
static CANCEL_FLAGS: std::sync::OnceLock<dashmap::DashMap<String, bool>> =
...
pub async fn prepare_chat_context(
    ...
    let agent_max_turns: usize = if let Some(ref aid) = agent_id {
        if let Some(agent_db) = claw_db::db::try_get_agent_db() {
            if let Ok(Some(agent)) =
                claw_db::db::agent_entities::agents::Entity::find_by_id(aid.to_string())
                    .one(agent_db)
                    .await
            {
    ...
    let _ = claw_rag::rag::compact_conversation_if_needed(
        ...
        Some(config.advanced.auto_compact_tokens),
    )
    .await;

    let rag_context =
        claw_rag::rag::build_rag_context(agent_id.as_deref(), conversation_id, user_message)
            .await
            .unwrap_or_default();
    ...
    log::info!(
        "[LLM] Loaded {} history messages ({}/{} tokens budget, RAG {} chars)",
        history.len(),
        used_tokens,
        history_budget,
        rag_context.len()
    ...
    let system_prompt = claw_rag::rag::build_system_prompt_with_agent(
        config,
        agent_id.as_deref(),
        Some(agent_max_turns),
        tool_count,
        Some(&tool_catalog),
    )
    .await;
    let tools: Vec<ToolDefinition> = tools_raw
        ...
        .collect();
    log::info!(
        "[LLM] {} tools registered | Provider: {} | Model: {} | MaxTurns: {}",
        tools.len(),
    ...
    let mut messages_for_api =
        PromptBuilder::build_messages(&history, &system_prompt, &rag_context, user_message);
...
fn inject_images_into_messages(
    ...
    if let Some(attachments) = images {
        ...
        if let Some(last_msg) = messages.last_mut() {
            ...
            for att in attachments {
                ...
                let b64 = if data_url.contains(',') {
                    data_url.splitn(2, ',').nth(1).unwrap_or("")
                } else {
                    data_url
                };
                if b64.is_empty() {
                ...
                if media_type.starts_with("image/") {
                    ...
                    other_media_info.push(format!(
                        "[File: {} | Type: {} | ~{}KB]",
                        name, media_type, size_kb
                    ));
                }
            }
                ...
                if let Some(text_part) = content_parts.iter_mut().find(|p| p["type"] == "text") {
                    if let Some(t) = text_part.get_mut("text") {
                        *t = serde_json::Value::String(format!(
                            "{}\n\n--- Attachments ({} files) ---\n{}\n--- End ---",
                            t.as_str().unwrap_or(""),
                            other_media_info.len(),
                            other_media_info.join("\n")
                        ));
                    }
                }
            }
...
async fn extract_agent_id(conversation_id: &str) -> Option<String> {
    if let Some(agent_db) = claw_db::db::try_get_agent_db() {
        if let Ok(sessions) = claw_db::db::agent_entities::agent_sessions::Entity::find()
            .filter(
                claw_db::db::agent_entities::agent_sessions::Column::ConversationId
                    .eq(conversation_id),
            )
            .all(agent_db)
            .await
...
pub async fn execute_tool(name: &str, input: &serde_json::Value) -> String {
    if let Some(executor) = crate::tool_executor::get_tool_executor() {
        ...
        log::error!(
            "[LLM] ToolExecutor not registered, cannot execute tool '{}'",
            name
        );
...
/// 将用户交互存储到RAG记忆系统
pub async fn store_interaction_to_rag(
    conv_id: &str,
    agent_id: Option<&str>,
    query: &str,
    response: &str,
    ...
    if let Some(aid) = agent_id {
        ...
        extract_world_facts(aid, conv_id, query, response)
            .await
            .map_err(|e| {
                log::warn!(
                    "[LLM:store_interaction_to_rag] extract_world_facts failed: {}",
                    e
                );
                e
            })
...
pub fn summarize_for_memory(text: &str, role: &str) -> String {
    ...
    for s in &sentences {
        if summary.len() + s.len() + 2 > max_len {
            break;
        }
        if !summary.is_empty() {
            summary.push_str(". ");
...
async fn extract_world_facts(
    ...
    for (pattern, category) in fact_patterns {
        if let Some(pos) = lower.find(pattern) {
            ...
            let fact_text = combined[start..end_byte]
                .lines()
                .next()
                .unwrap_or("")
                .trim()
            ...
            if !fact_text.is_empty() && fact_text.chars().count() > 3 {
                ...
                .map_err(|e| {
                    log::warn!(
                        "[LLM:extract_world_facts] store_enhanced_memory failed: {}",
                        e
                    );
                    e
                })
...
pub async fn send_chat_message(
    ...
    log::info!(
        "[LLM] conversation_id: {}, user_msg_len: {}",
    ...
    let (final_text, all_tool_calls, all_tool_executions, final_usage) = execute_tool_loop(
        ...
        conversation_id,
        user_message,
        &mut ctx.messages_for_api.clone(),
        &ctx.tools,
        None,
        false,
        Some(ctx.agent_max_turns),
    )
    .await
    .map_err(|e| anyhow::anyhow!(e))?;

    store_interaction_to_rag(
        conversation_id,
    ...
    .map_err(|e| {
        log::warn!(
            "[LLM:send_chat_message] store_interaction_to_rag failed: {}",
            e
        );
        e
    })
    ...
    if let Some(aid) = ctx.agent_id {
        claw_rag::rag::update_user_profile(&aid, user_message, &final_text)
            .await
            .map_err(|e| {
    ...
    log::info!(
        "[LLM] ========== END | text={} chars, tools={}, executions={} ==========",
        final_text.len(),
        all_tool_calls.len(),
...
pub async fn send_chat_message_streaming(
    ...
    log::info!(
        "[LLM:Stream] conv={}, msg_len={}, model={}, timeout={}s",
        truncate_str(conversation_id, 16),
        user_message.len(),
        config.model.default_model,
    ...
    if let Err(e) = super::streaming::emit_chat_stream(
        &app_handle,
        serde_json::json!({"type": "start", "conversation_id": conversation_id}),
    ) {
        log::error!("[LLM:Stream] Failed to emit 'start' event: {}", e);
        let _ = super::streaming::emit_chat_stream(
            &app_handle,
            serde_json::json!({"type": "error", "conversation_id": conversation_id, "content": format!("Failed to start stream: {}", e)}),
        );
    ...
    let ctx = match prepare_chat_context(config, conversation_id, user_message, images).await {
        ...
        Err(e) => {
            ...
            let _ = super::streaming::emit_chat_stream(
                &app_handle,
                serde_json::json!({"type": "error", "conversation_id": conversation_id, "content": err_msg.clone()}),
            );
            return Err(e);
        }
    };
    ...
    .map_err(|e| {
        log::warn!(
            "[LLM:stream_chat_message] store_interaction_to_rag failed: {}",
            e
        );
        e
    })
    .ok();
    if let Some(aid) = ctx.agent_id {
        claw_rag::rag::update_user_profile(&aid, user_message, &all_text)
            .await
            .map_err(|e| {
                log::warn!(
                    "[LLM:stream_chat_message] update_user_profile failed: {}",
                    e
                );
                e
            })
    ...
    log::info!(
        "[LLM:Stream] ========== END | text={} chars, executions={}, elapsed={:.2}s ==========",
        all_text.len(),
        all_tool_executions.len(),
        elapsed.as_secs_f64()
    );
...
pub async fn test_llm_connection_detailed(
    ...
    log_entry!(
        "INFO",
        "Config",
        format!("Provider: {}", config.model.provider)
    );
    log_entry!(
        "INFO",
        "Config",
        format!("Default Model: {}", config.model.default_model)
    );
        ...
        Ok(key) => {
            if key.is_empty() {
                log_entry!(
                    "ERROR",
                    "APIKey",
                    "Resolved API key is empty! Check Model settings or env vars."
                );
                return Ok(
                    serde_json::json!({ "success": false, "message": "API Key must not be empty (configure in Model settings)", "logs": logs }),
                );
            }
            let source = if !config.model.custom_api_key.is_empty() {
                "Model settings"
            } else {
                "env/config fallback"
            };
            log_entry!(
                "INFO",
                "APIKey",
                format!("API key resolved ({} chars, source: {})", key.len(), source)
            );
            key
        }
        Err(e) => {
            ...
            return Ok(
                serde_json::json!({ "success": false, "message": e.to_string(), "logs": logs }),
            );
        }
    };
    ...
    if base_url.is_empty() {
        ...
        return Ok(
            serde_json::json!({ "success": false, "message": "Model URL must not be empty", "logs": logs }),
        );
    }
    ...
    let model_name = if config.model.provider == "custom" {
        if config.model.custom_model_name.is_empty() {
            ...
            return Ok(
                serde_json::json!({ "success": false, "message": "Model Name must not be empty", "logs": logs }),
            );
        }
        log_entry!(
            "INFO",
            "Model",
            format!("Custom Model: {}", config.model.custom_model_name)
        );
        ...
        log_entry!(
            "INFO",
            "Model",
            format!("Model: {}", config.model.default_model)
        );
    ...
    log_entry!(
        ...
        "HTTPClient",
        format!(
            "Creating HTTP client (timeout={}s)...",
    ...
    let client = match client_result {
        Ok(c) => {
            log_entry!("INFO", "HTTPClient", "HTTP client created successfully");
            c
        }
        Err(e) => {
            log_entry!(
    ...
    let test_payload =
        "Hello! This is a connectivity test from Claw Desktop. Please respond briefly.";
    let result = if config.is_openai_compatible() {
        test_openai_connection_detailed(
            &client,
            &base_url,
            &api_key,
            &model_name,
        ...
        test_anthropic_connection_detailed(
            &client,
            &base_url,
            &api_key,
            &model_name,
...
async fn test_anthropic_connection_detailed(
    ...
    logs.push(TestLogEntry {
        ...
        level: "INFO".into(),
        phase: "Request".into(),
        detail: format!("Target URL: {}", url),
    });
    logs.push(TestLogEntry {
        timestamp: now(),
        level: "INFO".into(),
        phase: "Request".into(),
        detail: format!("API Version: {}", api_version),
    });
    ...
    logs.push(TestLogEntry {
        ...
        level: "INFO".into(),
        phase: "Network".into(),
        detail: "Sending POST request...".into(),
    });
    ...
    let response = client
        ...
        .map_err(|e| {
            ...
            let hint = if err_str.contains("dns") || err_str.contains("name") {
                "Hint: check network connection or DNS settings"
            } else if err_str.contains("refused") {
                "Hint: target server not responding"
            } else if err_str.contains("tls") {
                "Hint: SSL/TLS certificate issue"
            } else if err_str.contains("timeout") {
                "Hint: request timed out"
            } else {
                "Hint: check network/firewall/proxy"
            };
            logs.push(TestLogEntry {
                timestamp: now(),
                level: "ERROR".into(),
                phase: "Network".into(),
                detail: format!("Request failed: {} | {}", err_str, hint),
            });
    ...
    logs.push(TestLogEntry {
        ...
        level: "INFO".into(),
        phase: "Network".into(),
        detail: format!("Response received in {}ms", elapsed.as_millis()),
    });
    logs.push(TestLogEntry {
        timestamp: now(),
        level: "INFO".into(),
    ...
    if !response.status().is_success() {
        ...
        logs.push(TestLogEntry {
            ...
            level: "ERROR".into(),
            phase: "Response".into(),
            detail: format!("Error body: {}", error_text),
        });
    ...
    logs.push(TestLogEntry {
        ...
        level: "INFO".into(),
        phase: "Response".into(),
        detail: format!("Model confirmed: {}", model_resp),
    });
    logs.push(TestLogEntry {
        timestamp: now(),
        level: "INFO".into(),
        phase: "Response".into(),
        detail: format!("Reply text: {}", reply_text),
    });
    ...
    if let Some(u) = data.get("usage") {
        ...
        if let Some(cr) = u.get("cache_read_input_tokens").and_then(|v| v.as_u64()) {
            logs.push(TestLogEntry {
                ...
                level: "INFO".into(),
                phase: "Usage".into(),
                detail: format!("Cache read tokens: {}", cr),
            });
        }
    }
        ...
        for tc in &response.tool_calls {
            ...
            if let Ok(input_str) = serde_json::to_string(&tc.input) {
                if input_str.len() < 200 {
                    parts.push(format!("  参数: {}", input_str));
                }
            }
        }
        parts.join("\n")
    ...
    let response = client
        ...
        .map_err(|e| {
            ...
            let hint = if err_str.contains("dns") || err_str.contains("name") {
                ...
                "Hint: target server not responding, check address"
            } else if err_str.contains("tls") || err_str.contains("certificate") {
                "Hint: SSL/TLS certificate verification failed"
            } else if err_str.contains("timeout") || err_str.contains("timed out") {
                "Hint: request timed out, server too slow"
            } else {
                "Hint: check network/firewall/proxy settings"
            };
            logs.push(TestLogEntry {
                timestamp: now(),
                level: "ERROR".into(),
