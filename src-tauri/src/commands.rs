...
pub fn save_config(
    ...
    config
        .save(parent)
        .map_err(|e| format!("Save failed (path={}): {}", cfg_path.display(), e))?;
...
pub async fn list_conversations() -> Result<Vec<serde_json::Value>, String> {
    let convs = Database::list_conversations()
        .await
...
pub async fn create_conversation(agent_id: Option<String>) -> Result<serde_json::Value, String> {
    let conv = Database::create_conversation(agent_id)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn send_message(
    ...
    Database::add_message(&conversation_id, "user", &content, None, None, None)
        .await
        .map_err(|e| e.to_string())?;

    let response: ChatResponse =
        send_chat_message(&config, &conversation_id, &content, images.as_deref())
            .await
            .map_err(|e| e.to_string())?;
