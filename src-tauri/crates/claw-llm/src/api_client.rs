...
pub async fn call_openai_with_tools(
    client: &Client, base_url: &str, api_key: &str, config: &AppConfig,
    ...
    let key_preview = if api_key.len() > 8 { 
        ...
        let suffix: String = api_key.chars().skip(api_key.len().saturating_sub(4)).collect();
        format!("{}...{} (len={})", prefix, suffix, api_key.len()) 
    } else { 
        format!("(len={})", api_key.len()) 
    };
    log::info!("[LLM:OpenAI] POST {} key={} model={}", url, key_preview, config.model.default_model);
    ...
    let choice = data["choices"].as_array().and_then(|a| a.first())
    ...
    if let Some(arr) = message["tool_calls"].as_array() {
        for tc in arr {
            tool_uses.push(crate::llm::ToolCallInfo {
                ...
                name: crate::streaming::restore_tool_name_from_api(tc["function"]["name"].as_str().unwrap_or("")),
                input: tc["function"]["arguments"].as_str()
        ...
        for cb in arr {
            if cb["type"].as_str() == Some("tool_use") {
                tool_uses.push(crate::llm::ToolCallInfo {
                    ...
                    name: crate::streaming::restore_tool_name_from_api(cb["name"].as_str().unwrap_or("")),
