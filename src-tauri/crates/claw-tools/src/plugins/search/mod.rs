...
#[tauri::command]
pub fn tool_glob(pattern: String, path: Option<String>, exclude_patterns: Option<Vec<String>>) -> Result<serde_json::Value, String> {
    ...
    let entries: Vec<String> = glob(&full_pattern)
        ...
        .filter(|entry: &PathBuf| {
            ...
            !excludes.iter().any(|excl: &String| p.contains(excl.as_str()))
        })
    ...
    if entries.is_empty() {
        ...
        let out = format!("Found {} matches for '{}':\n{}", entries.len(), pattern, entries.join("\n"));
        Ok(serde_json::json!({"tool":"Glob","success":true,"output":out}))
    }
}
...
#[tauri::command]
pub fn tool_grep(pattern: String, path: Option<String>, include_pattern: Option<String>, exclude_pattern: Option<String>) -> Result<serde_json::Value, String> {
    ...
    fn find_files(dir: &PathBuf, depth: usize) -> Vec<PathBuf> {
        ...
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                else if p.is_dir() { files.extend(find_files(&p, depth + 1)); }
            }
        }
        files
    }
    ...
    for file_path in find_files(&search_dir, 0) {
        ...
        if let Some(ref inc) = include_re { if !inc.is_match(&fname) { continue; } }
        if let Some(ref exc) = exclude_re { if exc.is_match(&fname) { continue; } }
        if let Ok(meta) = std::fs::metadata(&file_path) { if meta.len() > 10 * 1024 * 1024 { continue; } }
        if let Ok(content) = std::fs::read_to_string(&file_path) {
            for (idx, line) in content.lines().enumerate() {
                if re.is_match(line) {
                    results.push(format!("{}:{}: {}", file_path.display(), idx + 1, line.trim()));
    ...
    if results.is_empty() {
        ...
        let out = format!("Found {} matches '{}':\n{}", results.len(), pattern, display_results);
        Ok(serde_json::json!({"tool":"Grep","success":true,"output":out}))
    }
}
