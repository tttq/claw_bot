...
pub async fn hook_list(
    ...
    match registry.load_from_db().await {
        Ok(_) => {
            ...
            Json(ApiResponse::ok(serde_json::to_value(hooks).unwrap_or_default()))
        }
...
pub async fn hook_create(
    ...
    let event = match claw_harness::harness::hooks::HookEvent::from_str_value(event_str) {
        Some(e) => e,
        None => return Json(ApiResponse::err(&format!("Invalid hook event: {}", event_str))),
    };
    ...
    let hook = claw_harness::harness::hooks::HookDefinition {
        ...
        name: params.get("name").and_then(|v| v.as_str()).unwrap_or("Untitled Hook").to_string(),
        event,
        pattern: params.get("pattern").and_then(|v| v.as_str()).map(String::from),
        handler_type: params.get("handler_type").and_then(|v| v.as_str()).unwrap_or("log").to_string(),
        handler_config: params.get("handler_config").cloned().unwrap_or(serde_json::Value::Object(Default::default())),
